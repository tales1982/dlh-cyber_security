...
# Capstone (2x05) fix: OUTPUT and CVE_FEED are hardcoded to SCRIPT_DIR in
# the original 2x03 script, so cd-based directory redirection alone cannot
# move them into capstone/patch/ or point CVE_FEED at the capstone-provided
# feed. CAPSTONE_ARTIFACTS_DIR and CAPSTONE_CVE_FEED let the Task 6
# orchestrator redirect both while every other pipeline stage script (which
# already writes CWD-relative output) needs no override at all.
if [[ -n "${CAPSTONE_ARTIFACTS_DIR:-}" ]]; then
    OUTPUT="${CAPSTONE_ARTIFACTS_DIR}/vulnerability_inventory.json"
else
    OUTPUT="${SCRIPT_DIR}/vulnerability_inventory.json"
fi
CVE_FEED="${CAPSTONE_CVE_FEED:-${SCRIPT_DIR}/cve_feed.json}"
