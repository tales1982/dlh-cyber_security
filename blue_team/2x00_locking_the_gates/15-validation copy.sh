...
record() {
    ...
    obj=$(jq -n \
        ...
        --arg actual "$actual" \
        --argjson ok "$([ "$ok" -eq 1 ] && echo true || echo false)" \
        '{
            control_id: $cid,
            check: $label,
            expected: $expected,
...
check_cfg() {
    ...
    val=$(
        grep -iE "^\s*${key}\s*[= ]" "$file" 2>/dev/null |
        tail -1 |
...
for f in "${suid_bins[@]}"; do
    echo "$f" | grep -qE "$whitelist_regex" ||
...
if [ -r /etc/audit/rules.d/meddefense.rules ]; then
    rule_count=$(
        grep -cE '^-w ' /etc/audit/rules.d/meddefense.rules 2>/dev/null ||
        true
    )
fi
...
if [ -r /etc/audit/rules.d/meddefense.rules ] &&
   grep -q 'priv_esc' /etc/audit/rules.d/meddefense.rules 2>/dev/null; then
    priv_esc_present=1
fi
...
if [ -n "$audit_validation_file" ]; then
    executed=$(
        jq -r '.summary.tests_executed // 0' \
            "$audit_validation_file" 2>/dev/null
    )

    captured=$(
        jq -r '.summary.captured // 0' \
            "$audit_validation_file" 2>/dev/null
    )
